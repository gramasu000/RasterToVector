""" FILE: FloorplanTransformation/code/datasets/floorplan-representation.lua """
...
# end
""" PYTHON """
    __init__(self, imageInfo, opt, split):
        self.imageInfo = imageInfo[split]
        if split == "train":
            self.imageInfo.floorplanPaths = self.imageInfo.floorplanPaths.repeatTensor(checkpointEpochInterval, 1)
            self.imageInfo.representationPaths = self.imageInfo.representationPaths.repeatTensor(checkpointEpochInterval, 1)
        self.opt = opt
        self.split = split
        if (os.)
        self.dir = 
